...
/**
 * The pattern database for the first 6 edge cubies.
 * @see PatternDatabase
 */
public class SmallFirstEdgePatternDatabase extends PatternDatabase {
    
    // For this database, we are looking at the first 6 edges out of 12, so n=12 and k=6
    ...
    /**
     * Constructor for the first edge pattern database.
     * Sets the database size to 42577920, n to 12, and k to 6.
     */
    public SmallFirstEdgePatternDatabase() {
    ...
    protected int getDatabaseIndex(Cube cube) {
        ...
        // Get the first 6 edge indices and orientations from the cube
        byte[] edgeIndices = new byte[6];
        byte[] edgeOrientations = new byte[6];

        // Iterate over edges until the first 6 are found
        int counter = 0;
        for (byte i = 0; i < 12; i++) {
